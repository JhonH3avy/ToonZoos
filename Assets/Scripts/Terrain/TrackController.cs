...
public class TrackController : MonoBehaviour
{
    ...
    public static TrackController Instance
    {
        get
        {
            if ( instance == null )
            {
                instance = GameObject.FindObjectOfType<TrackController> ();
    ...
    public delegate void RaceFinishtHandler (FinishingRaceEventArgs args);
    ...
    private void OnTriggerEnter (Collider c)
    {
        if ( c.transform.tag.Contains ( "Player" ) && !isFinished )
        {
            ...
            if ( FinishingRace != null )
                FinishingRace ( new FinishingRaceEventArgs ( c.gameObject ) );
            isFinished = true;
        }
    }
}

public class FinishingRaceEventArgs : System.EventArgs
{
    public FinishingRaceEventArgs (GameObject winner)
    {
        this.Winner = winner;
    }
